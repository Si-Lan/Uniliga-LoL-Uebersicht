...
echo "<br>---- getting unplayed Matchresults from Toornament<br>";
$matches = $dbcn->query("SELECT * FROM matches AS m INNER JOIN `groups` AS g ON g.GroupID = m.GroupID INNER JOIN divisions AS d ON d.DivID = g.DivID WHERE TournamentID = {$tournament_id} AND m.played = FALSE")->fetch_all(MYSQLI_ASSOC);
